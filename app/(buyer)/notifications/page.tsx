...
export default function NotificationsPage() {
  ...
  const getNotificationIcon = (type: string) => {
    switch (type) {
      ...
      case 'MESSAGE':
        return <MessageCircle className="w-5 h-5" style={{ color: colors.primary }} />
      default:
  ...
  const getNotificationLink = (notification: Notification | MessageNotification) => {
    if (notification.type === 'ORDER' && (notification as any).data?.orderId) {
      return `/orders/${(notification as any).data.orderId}`
    }
    if (notification.type === 'PAYOUT' && (notification as any).data?.payoutRequestId) {
      return `/seller/wallet`
    }
    if (notification.type === 'MESSAGE' && (notification as MessageNotification).conversationId) {
      return `/messages?conversationId=${(notification as MessageNotification).conversationId}`
    }
    return '#'
  }
